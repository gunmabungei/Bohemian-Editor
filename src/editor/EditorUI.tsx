...
const worksPending: Works =
    {
        id: 1, body: "loading...", postscript: "loading...",
        author: "loading...", title: "loading...", pages: 'xxx'
    }
...
export default function EditorUI() {
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE_URL}/journal/${params.journal_name}/works/${fetchId}`)
            ...
            .then((data: IWorks)=> {
                setWorksList(
                    worksList.map((c, i) => { return (i === data.index - 1) ? data : c })
                )
            })
    ...
    const saveChanges = () => {
        fetch(`${API_BASE_URL}/journal/${params.journal_name}/works/${worksList[selector].id}`,
            {
                method: "POST",
        ...
        fetch(`${API_BASE_URL}/journal/works_list/${params.journal_name}`)
            ...
            .then(data => {
                data = data
                        ...
                        .map((x: IWorks) => {x.pages="xxx"; return x})
                        .map((x: IWorks) => {return {...x, index: Number(x.index)}})
                        .map((x: IWorks) => {return {...x, id: Number(x.id)}})
                setWorksList(data.sort((a: IWorks, b: IWorks) => a.index > b.index))
            })
              ...
              >
                  ...
                  >
                      <WorksList
                          ...
                          selection={selector}
                      />
                      <Group mt={"auto"} gap="6px" grow w={"320"}
                  ...
                  >
                      <Editor works={worksList[selector]} onSave={saveChanges} editWorks={editWorks} />
                  </AppShell.Main>
              </AppShell>
          </MantineProvider>
      </>
