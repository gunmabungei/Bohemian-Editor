...
export const AddWorks = ({ onSubmit, onComplete }: AddWorksProps) => {
    ...
    const [values, setValues] = useState<FormParam>({ files: undefined, body: '', title: '', author: '', postscript: '' });
    const handleClick = () => {
        onSubmit()
        submitForm(param.journal_name ?? "test", values)
            .then(onComplete)
    }
    ...
    return (
        <>
            <AddWorksForm tabs={tabs} onSubmit={handleClick} onChangeTabs={handleChanges} formValues={values} setFormValue={setValues} />
